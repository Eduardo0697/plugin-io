...
class BaseSearchFactory
{
    ...
    public static function inherit($searchBuilder, $inheritedProperties = null)
    {
        ...
        if ($searchBuilder !== null) {
            if ($inheritedProperties === null || in_array(self::INHERIT_COLLAPSE, $inheritedProperties) && !is_null(
                    $searchBuilder->collapseField
                )) {
    ...
    {
        $this->addSorting($this->sorting, $field, $order);
        return $this;
    }
     ...
     * @return $this
     */
    public function sortInnerBy($field, $order = self::SORTING_ORDER_DESC)
    {
        $this->addSorting($this->innerSorting, $field, $order);
        return $this;
    }

    /**
     * Add multiple inner sorting parameters
     *
     * @param array $sortingList List of sorting parameters. Each entry should have a 'field' and an 'order' property.
     *
     * @return $this
     */
    public function sortInnerByMultiple($sortingList)
    {
        foreach ($sortingList as $sorting) {
            $this->sortInnerBy($sorting['field'], $sorting['order']);
        }
    ...
    protected function prepareSearch($source)
    {
        ...
        if (!is_null($this->collapseField)) {
            ...
            $counterAggregation = pluginApp(
                ItemAttributeValueCardinalityAggregation::class,
                [$counterAggregationProcessor, $this->collapseField]
            );
            ...
            if(!is_null($this->innerSorting)) {
                $innerHit->setSorting($this->innerSorting);
            }
    ...
    private function addSorting(&$sorting, $field, $order = self::SORTING_ORDER_DESC)
    {
        if(is_null($sorting)) {
            /** @var MultipleSorting $sorting */
            $sorting = pluginApp(MultipleSorting::class);
        }
        ...
        if(!is_null($sortingInterface)) {
            $sorting->addSorting($sortingInterface);
        }
    }
}
